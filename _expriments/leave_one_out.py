...
def leave_one_out(instances, llm):    
    ...
    for key, val in instances.items() :
        ...
        if (count < 73):
            ...
            response = rank(query=question, index=index, llm=llm)
...
instances = util.load_instances(
    "MnZcDescriptor\data\model_checkpoints\leave_one_out_full.pkl"
)
...
leave_one_out(instances=instances, llm=llm)
